...
## function to wait for4 any voice input
def speak(audio):
    engine.say(audio)   
    engine.runAndWait()

## function for using the speech recogniser and google to parse the input command to set up next fuction of looking for specific words
def command():
    r = sr.Recognizer()
    while True:
